...
那么我们如何找最低能量的构象呢？首先我们需要`cat gibbs.log`然后从中找到最低能量对应的minimum和index索引组，假如说索引组是141，那么我们就`cat bindex.ndx`，然后从中找到索引[ 141 ]，查看到这个索引对应的时间帧是第4帧和第1282帧，然后我们可以根据这个帧数去计算对应的模拟的时刻，从而使用`gmx trjconv`命令提取对应时刻的蛋白质构象。

数据的开始时刻和时间间隔都可以从`sham.xvg`里面看到，所以实际上第几帧也就是`sham.xvg`的数据的第几行，那么对应的从第几行就可以获得对应的时间帧，这是一种简洁的时间帧获得方法。
